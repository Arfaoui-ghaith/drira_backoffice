...
export default function Landing() {
    ...
    const deleteLanding = async () => {
		try {
			console.log(landing);
			const url =`https://spnetwork.herokuapp.com/api/v1/landings/${landing._id}`;
			const res = await axios({
								...
								<Table responsive>
									...
									{ landings.map((land, index) => (
										<tr key={index}>
                                            ...
                                            <td>
                                            	<img src={`https://spnetwork.herokuapp.com/images/landings/${land.image}`} className="rounded" style={{"height": "60px"}} />
                                            </td>
                                            <td>
                                            	{land.title}
                                            </td>
